...
void main() {
  group('Round 22 — scheduled-playback volume + controls + mini-player bridge', () {
    test('WhisperPlaybackService publishes pause/resume/stop actions in its notification', () {
      final src = _read(
      ...
      expect(src, contains('handleResumeCommand'),
          reason: 'Resume command must restart MediaPlayer and update the notification.');
    });

    test('WhisperPlaybackService honours a user-set playback volume from SharedPrefs', () {
      final src = _read(
          'android/app/src/main/kotlin/com/whisperback/whisperback/alarms/WhisperPlaybackService.kt');
      ...
      expect(src, contains('KEY_VOLUME'),
          reason: 'Stable preference key for the volume so the bridge and service agree.');
    });

    test('WhisperPlaybackService mirrors state into SharedPrefs + invokes the Dart listener', () {
      final src = _read(
          'android/app/src/main/kotlin/com/whisperback/whisperback/alarms/WhisperPlaybackService.kt');
      ...
      expect(src, contains('writeState(STATE_IDLE)'),
          reason: 'Idle write on stop / completion clears the cold-start snapshot.');
    ...
    test('MainActivity exposes pauseNative / resumeNative / stopNative / setVolume / getPlaybackState', () {
      final src =
          _read('android/app/src/main/kotlin/com/whisperback/whisperback/MainActivity.kt');
      ...
      expect(src, contains('android.os.Handler(android.os.Looper.getMainLooper())'),
          reason:
    ...
    test('NativeAlarmsBridge exposes the new control + poll APIs', () {
      ...
      expect(src, contains('pauseNative'),
          reason: 'Coordinator calls pauseNative on pause when the native source is active.');
      expect(src, contains('resumeNative'),
          reason: 'Coordinator calls resumeNative on resume when the native source is active.');
      expect(src, contains('stopNative'),
          reason: 'Coordinator calls stopNative on dismiss / stop when the native source is active.');
      expect(src, contains('setVolume'),
          reason: 'A future volume slider needs an entry point even if no UI ships in this round.');
      expect(src, contains('fetchPlaybackState'),
          reason: 'Cold-start poll API must exist for the mini-player to recover a mid-flight scheduled clip.');
      expect(src, contains('stateStream'),
          reason: 'PlaybackCoordinator subscribes to this stream to drive the mini-player.');
      expect(src, contains('NativePlaybackSnapshot'),
          reason: 'A typed snapshot is required so coordinator logic isn\'t string-typed.');
    });

    test('PlaybackCoordinator routes pause/resume/dismiss/stop to native when scheduled playback is active', () {
      final src = _read('lib/services/playback/playback_coordinator.dart');
      expect(src, contains('_nativeScheduledActive'),
          reason: 'Coordinator must track whether the visible snapshot is a native scheduled clip.');
      ...
      expect(src, contains('NativeAlarmsBridge.instance.resumeNative()'),
          reason: 'Resume tap must reach the native FG service for the same reason.');
      ...
      expect(src, contains('_onNativePlaybackState'),
          reason: 'Coordinator must observe native transitions so the mini-player can light up on schedule fires.');
